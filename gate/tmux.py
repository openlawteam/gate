...
Provides kill_window for cancellation and cleanup, along with standard
pane capture, key sending, and window management helpers.
"""
...
def new_window(
    ...
    try:
        ...
        logger.error("tmux new-window failed (not found or timeout)")
        return None
...
def capture_pane(target: str) -> str | None:
    """Capture the contents of a tmux pane as plain text.

    Args:
        target: The tmux target (pane ID like "%1" or window ID like "@1").

    Returns:
        The pane contents as plain text, or None on failure.
    """
    try:
        result = subprocess.run(
...
def kill_window(target: str) -> bool:
    """Kill the tmux window containing the given pane.

    Used for cancellation (re-push kills in-flight review) and cleanup.

    Args:
        target: The tmux target (pane ID like "%1").

    Returns:
        True if the window was killed, False otherwise.
    """
    try:
        result = subprocess.run(
