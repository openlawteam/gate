# Implement

Implement the task described below based on the plan we've reviewed.

1. **Work through it carefully** — clean, maintainable code. KISS and DRY.

2. **Verify when complete** — run `npx tsc --noEmit 2>&1 | tail -30` and `npm run lint:check 2>&1 | tail -30` to check your work. Fix errors in files you touched.

3. **Stay focused** — only implement what was planned. No extra features or embellishments.

## Constraints

- Do NOT modify files matching the blocklist:
$blocklist
- Do NOT run git commands
- Do NOT add or remove npm dependencies

## Output

Summarize your work:

- Files changed and what was done
- Verification results (tsc/lint output)
- Any issues or followups encountered

Implementation complete — ready for review.

---

## Directions

$request
